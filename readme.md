...
f) **imageFunctions:**
- screenshot(fileName:str, region:[left:int, top:int, width:int, height:int]) -> Makes a screenshot of requested area and saves it to file given in fileName.
- locateImage(fileName:str, region:None, grayscale:Bool): [int, int] -> Checks if image passed by fileName is present in region of screen. Returns sequence of ints (left, top) or equence of strings ('None', 'None') if there is no match. Grayscale forces images to be grayscaled which improves timing at the expense of accuracy.
